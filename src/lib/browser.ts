...
export const BROWSER_CAVEATS: Array<{ title: string; detail: string }> = [
  ...
  },
  {
    ...
    detail:
      "Deliberately, for now. Each one is its own project rather than something added on the way past. Bookmarks, find-in-page, a downloads list and history all work — history skips private workspaces and the assistant's own tabs, by design rather than by setting. The browser itself speaks English and Dutch; it asks which on first run.",
  },
];
