...
--
-- It is the migrations 0003 to 0017 concatenated, in order, verbatim; the
...
-- ─── from 0017-a-message-that-arrives.sql ──────────────────────────
...
select 'chat' as area,
       (select count(*) from pg_tables
         where schemaname = 'public'
           and tablename in ('channels','channel_members','messages')) as tables,
       (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public'
           and p.proname in ('in_channel','open_dm','toggle_reaction')) as functions;
